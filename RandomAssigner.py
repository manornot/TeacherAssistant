...
HW_path = Path('/mnt/g/Other computers/PC/Drive/Ju/EDX/Python_coding/DiscordBot/Students_HW')
assignments_path = Path(
    "G:\Other computers\PC\Drive\Ju\EDX\Python_coding\DiscordBot\Assignments")
assignments_path = Path('/mnt/g/Other computers/PC/Drive/Ju/EDX/Python_coding/DiscordBot/Assignments')


hws_folders = os.listdir(HW_path)
hws_fldrs_paths = []
hws_paths = []
for hw_fldr in hws_folders:
    hws_fldrs_paths.append(HW_path.joinpath(f'{hw_fldr}'))

for pth in hws_fldrs_paths:
    hws_paths.append([os.path.join(pth, file) for file in os.listdir(pth)])
...
for directory,pth_list,indx in zip(hws_fldrs_paths,hws_paths,range(len(hws_paths))):
    ...
    if len(pth_list) > 1:
        fl_nm = shutil.make_archive(directory,format='zip',root_dir=directory)
...
for flrd in hws_folders:
    os.mkdir(assignments_path.joinpath(f'{flrd}'))

random.shuffle(hws_paths)

student_id_map = StudenIDMap(studentList=[studentAssignemt(name=student[0], std_id=index)
                                          for index, student in enumerate(hws_paths)])
...
for indx,student in enumerate(student_id_map.studentList):
    probability_list = np.asanyarray([1/len(np.unique(available_assignment_list))]*len(np.unique(available_assignment_list)))
    logger.debug(f'probability sum = {np.sum(probability_list)}')
    probability_list[indx] = 0
    probability_list[probability_list>0]+=(1-np.sum(probability_list))/(len(probability_list)-1)
    logger.debug(f'probability sum = {np.sum(probability_list)}')
    student.assignments = list(np.random.choice(np.unique(available_assignment_list),3,replace=False,p=probability_list))
"""
...
for stud in student_id_map.studentList:
    for assignmnt in stud.assignments:
        ...
        logger.debug(f'file name = {file_name}, file extension = {file_extension},')
        hw_dest = assignments_path.joinpath(dir_name).joinpath(f'{assignmnt}{file_extension}')
